...
class Leetcode(object):
    ...
    def submit(self, slug_name, solution):
        # Get question id
        ...
        with subprocess.Popen(['leetcode', 'exec', str(question_id)], stdout=subprocess.PIPE, encoding='utf-8') as process:
            process.wait(timeout=10)  # Returns only code: 0
        ...
        status = outs[1] 
        runtime, runtime_precentage, memory, memory_precentage = None, None, None, None
        if status == "Success":
            rs, ms = outs[3].split(" "), outs[5].split(" ")
            runtime, memory = rs[1], ms[2]
            runtime_precentage, memory_precentage = rs[5], ms[6]
        
        return status, runtime, runtime_precentage, memory, memory_precentage
    
if __name__ == "__main__":
    leetcode = Leetcode("Python")
    
    slug_name = "minimum-sum-of-mountain-triplets-ii"
    with open(f"{leetcode.solution_dir}/{slug_name}.py", "r") as solution_f:
        solution = solution_f.read()
    result = leetcode.submit(slug_name, solution)
